...
func (h *TencentCloudCertHandler) Handle(ctx context.Context, e caddy.Event) error {
	...
	if slices.Contains(h.BlockList, identifier) || len(h.AllowList) > 0 && !slices.Contains(h.AllowList, identifier) {
		h.logger.Info(fmt.Sprintf("upload_cert_tencentcloud ignored certificate %s not matching the current rule", identifier), zap.String("event", e.Name()))
		return nil
	}
	...
	go func() {
		certificateId, _ := h.DescribeCertificates(ctx, identifier)
		if certificateId == "" {
			if err := h.UploadCertificate(ctx, cert, key); err != nil {
				h.logger.Error("upload certificate failed", zap.String("certificate", identifier), zap.Error(err))
				return
			}
		} else {
			...
			for i := 0; i < 60 && deployRecordId == 0; i++ {
				if err := h.UpdateCertificateInstance(ctx, cert, key, certificateId, &deployRecordId); err != nil {
					if err := h.UploadCertificate(ctx, cert, key); err != nil {
						h.logger.Error("upload certificate after update failure failed", zap.String("certificate", identifier), zap.Error(err))
						return
					}
					deployRecordId = 1
				}
